...
and print_condition = function 
	| Conditionelle(t_terma, t_ope ,t_termb) -> print_terminal(t_terma);print_operateur(t_ope);print_terminal(t_termb);
...
let rec  print_instr structprog = match structprog with
	| If(cond)::tl -> output_string oc ("if ");print_condition(cond); print_instr tl
	| ElseIf(cond)::tl -> output_string oc ("else if ");print_condition(cond); print_instr tl
